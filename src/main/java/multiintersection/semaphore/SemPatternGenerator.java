package multiintersection.semaphore;
...
public class SemPatternGenerator {
	...
	public static Set<String> generatePatterns(int flows, int totalLength, int minimumSlot, int advancement,
			int lastFlow) {
		if (flows > 10 || flows < 1)
			throw new IllegalArgumentException("Questo generatore non funziona con pi� di dieci flussi (o meno di 1).");
	 ...
	 * {@param totalLength} � suddiviso in slot di lunghezza {@param minimumSlot}.
	 * L'assegnazione di ogni slot � sorteggiata casualmente tra uno dei flussi
	 * {@param flows}.
	 * <p>
	...
	/**
	 * Metodo riunione 04/10/2022 (rivelatosi cattivo, perch� privilegia gli slot
	 * piccoli)
	 */
	...
	private static String generateRandomPatternWithSlots(int flows, int period, List<Integer> slotDurations) {
		...
		while (sum < period) {
			...
			switch (maxSlotIndex) {
			case -1: // nessuno slot � abbastanza piccolo da poter essere collocato, il pi� corto
						// viene allungato
			{
			...
			case 0: // solo lo slot pi� piccolo pu� essere collocato
			{
				chunks.add(slotDurations.get(0));
				sum += chunks.get(chunks.size() - 1);
				break;
			}
			default: // pi� di uno slot pu� essere collocato, quindi occorre sorteggiare
			{
	...
	public static String generateRandomPatternWithRedTime(int flows, int period, int minimumSlot, int redTime) {
		if (flows > 9 || flows < 1)
			throw new IllegalArgumentException("Questo generatore non funziona con pi� di nove flussi (o meno di 1).");
		...
		while (sb.length() < period - 2 * redTime - minimumSlot) {
			...
			if (firstFlow == -1)
				firstFlow = i;
			// se il flusso sorteggiato � diverso dal precedente, assegna un periodo di
		...
		if (previousFlow == firstFlow) {
			// se l'ultimo slot � assegnato allo stesso flusso che al primo non serve il
			// periodo di rosso per tutti
			while (sb.length() < period) {
				sb.append(previousFlow);
			}
		} else { // l'ultimo flusso � diverso dal primo, quindi occorre il periodo di rosso
			// se non c'� spazio per il periodo di rosso, l'ultimo slot viene accorciato
